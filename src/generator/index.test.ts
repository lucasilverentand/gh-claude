...
describe('WorkflowGenerator', () => {
  ...
  describe('generate', () => {
    ...
    it('should generate workflow with custom Claude configuration', () => {
      ...
      expect(claudeStep?.run).toContain('claude-opus-4');
      expect(claudeStep?.run).toContain('8000');
      expect(claudeStep?.run).toContain('0.5');
    });
    ...
    it('should generate workflow with allowed-paths for file operations', () => {
      ...
      // Verify skills documentation includes allowed paths
      const prepareSkillsStep = workflow.jobs['claude-agent'].steps.find(
        (s) => s.name === 'Prepare skills documentation'
      );
      expect(prepareSkillsStep).toBeDefined();
    });
  ...
  describe('writeWorkflow', () => {
    it('should write workflow to file system', async () => {
      ...
      expect(outputPath).toContain('test-agent.yml');
    ...
    it('should sanitize agent names for file paths', async () => {
      ...
      expect(outputPath).toContain('test-agent-with-spaces-special-chars.yml');
    });
  });
