...
export class ContextCollector {
  /**
   * Generates a bash script that collects context from GitHub
  ...
  generateCollectionScript(config: ContextConfig): string {
    ...
    const contextScripts: Array<string | null> = [
    ...
    scriptParts.push(`echo "has-context=true" >> $GITHUB_OUTPUT`);
    scriptParts.push(``);
    scriptParts.push(`# Save collected data to file for next job`);
    scriptParts.push(`echo "$COLLECTED_DATA" > /tmp/context.md`);
    scriptParts.push(``);
    ...
    scriptParts.push(`echo "context-data<<EOF" >> $GITHUB_OUTPUT`);
...
if [ "${since}" = "last-run" ]; then
  ...
  private generateIssuesScript(config: IssuesContextConfig): string {
...
if [ "$ISSUES_COUNT" -gt 0 ]; then
  ...
  private generatePullRequestsScript(config: PullRequestsContextConfig): string {
    const limit = config.limit || 100;
    // GitHub API only accepts 'open', 'closed', or 'all' - not comma-separated values
    const state = this.normalizeState(config.states);
    const labels = config.labels?.join(',') || '';
...
if [ "$COMMITS_COUNT" -gt 0 ]; then
  ...
  private generateReleasesScript(config: ReleasesContextConfig): string {
