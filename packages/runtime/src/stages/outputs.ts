...
async function executeOutputs(
  ...
  for (const file of files) {
    try {
      await executeOutput(outputType, file, config, ctx);
      executed++;
...
async function executeAddComment(
  ...
  const footer = `\n\n> *Generated by workflow [${process.env.GITHUB_WORKFLOW}](${process.env.GITHUB_SERVER_URL}/${repository}/actions/runs/${process.env.GITHUB_RUN_ID})*`;
...
async function executeAddLabel(
  ...
  // Update labels via API - use echo to pipe JSON to gh api
  const labelsJson = JSON.stringify(mergedLabels);
  await $`echo ${labelsJson} | gh api repos/${repository}/issues/${issueOrPrNumber}/labels -X PUT --input -`;
}
...
async function executeRemoveLabel(
  ...
  await $`echo ${remainingJson} | gh api repos/${repository}/issues/${issueOrPrNumber}/labels -X PUT --input -`;
}
 ...
 */
async function executeCreateDiscussion(file: OutputFile, repository: string): Promise<void> {
  const title = file.data.title as string;
  const body = file.data.body as string;
...
async function executeUpdateFile(file: OutputFile, repository: string): Promise<void> {
  ...
  for (const fileSpec of files) {
    ...
    // Update file via API - use echo to pipe JSON to gh api
